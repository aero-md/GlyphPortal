<script lang="ts">
  ...
  import { buildGeometry } from "../matrix/matrix";

  /** Côté de la matrice d'une capitale, et son masque — celui des appareils,
      littéralement : la même fonction, à une autre taille. Centre au milieu,
      `d < r`, les coins n'existent pas. 37 cellules sur 49.

      Sonoglyph en gardait une réimplémentation de six lignes, faute d'avoir
      `buildGeometry` sous la main. Elle donnait le même masque — jusqu'au jour
      où l'une des deux aurait bougé. */
  const M = 7;
  const MASK = buildGeometry(M).isInside;
  ...
  const GLYPHS: Record<string, string[]> = {
    ...
    S: [".......", "..####.", ".#.....", "..###..", ".....#.", ".####..", "......."],
    T: [".......", ".#####.", "...#...", "...#...", "...#...", "...#...", "......."],
    /* D et B suivent la panse du P : même fût, même largeur d'anse. Leurs
       rangées extrêmes s'arrêtent en colonne 4, ce que le disque autorise —
       elles ouvrent les colonnes 1 à 5. */
    D: [".......", ".####..", ".#...#.", ".#...#.", ".#...#.", ".####..", "......."],
    B: [".......", ".####..", ".#...#.", ".####..", ".#...#.", ".####..", "......."],
    /* O est le C refermé, et non un D aux angles adoucis : à cette taille la
       différence entre les deux tient au fût, pas à la rondeur. N porte sa
       diagonale sur les trois rangées centrales, une colonne par rangée — c'est
       tout ce que 5 colonnes permettent entre deux fûts, et il en faut trois
       pour que la pente se lise comme une pente et non comme un accident. */
    O: [".......", "..###..", ".#...#.", ".#...#.", ".#...#.", "..###..", "......."],
    N: [".......", ".#...#.", ".##..#.", ".#.#.#.", ".#..##.", ".#...#.", "......."],
    /* E : les trois barres du F plus le pied. La médiane est raccourcie d'une
       colonne — à cette taille c'est le seul signal qui distingue le E du B
       une fois les panses fermées, et l'allonger jusqu'en colonne 4 le rendait
       aveugle. Les barres extrêmes s'arrêtent en colonne 4, ce que le disque
       autorise sur les rangées 1 et 5. */
    E: [".......", ".####..", ".#.....", ".###...", ".#.....", ".####..", "......."],
  };
  ...
  let { text = "SONOGLYPH", cell = 5.7 }: Props = $props();
...
<style>
  canvas {
    ...
    color: var(--ink);
    /* SONOGLYPH fait 51 colonnes de trame, soit ~291 px : ça tient sur un écran
       de 360 mais plus sous 330. Plutôt que de rapetisser `cell` pour tout le
       monde — et de passer sous le plancher où les points se rejoignent — on
       laisse le canvas se réduire lui-même dans ce seul cas. */
    max-width: 100%;
    height: auto;
  }
</style>
